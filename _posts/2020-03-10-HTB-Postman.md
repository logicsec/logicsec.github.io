---
layout: post
title: "Hack The Box Walkthrough: Postman"
custom_page_description: "Walkthrough of HTB: Postman"
categories: htb
post_image: "/assets/htb-postman/infocard.PNG"
...
<p>Postman was an easy level box with with an interesting foothold. We started with an nmap scan and discovered a redis server and a webmin service. From there we used an existing exploit for redis to gain SSH access. From there we discovered SSH keys that were backed up. To gain root, we go back to the webmin services and use an existing exploit to get ourselves a root shell.</p>
